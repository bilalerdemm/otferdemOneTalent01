...
CLASS lcl_main_controller DEFINITION CREATE PRIVATE FINAL.
  ...
  PRIVATE SECTION.
    CLASS-DATA:
      ...
      "! ALV List
      mt_alv_list_sat              TYPE TABLE OF zot_07_s_sat,
      mt_alv_list_sas              TYPE TABLE OF zot_07_s_sas,
...
*    ENDIF.
  ...
  METHOD get_data_sat.
    ...
    SELECT eban~banfn,
           ...
           eban~menge,
           eban~meins
    FROM eban INNER JOIN ekpo ON eban~banfn = ekpo~banfn AND eban~bnfpo = ekpo~bnfpo INTO CORRESPONDING FIELDS OF TABLE @me->mt_alv_list_sat
    WHERE eban~banfn IN @s_banfn AND eban~bsart = @s_bsart.
...
*    ENDLOOP.
  ...
  METHOD get_data_sas.
    ...
    SELECT ekpo~ebeln,
           ...
           ekpo~menge,
           ekpo~meins
    FROM eban INNER JOIN ekpo ON eban~banfn = ekpo~banfn AND eban~bnfpo = ekpo~bnfpo INTO CORRESPONDING FIELDS OF TABLE @me->mt_alv_list_sas
    WHERE ekpo~ebeln IN @s_ebeln AND ekpo~matkl IN @s_matkl.
...
*    ENDLOOP.
  ...
  METHOD display_grid_sat.
    ...
    ASSIGN me->mt_alv_list_sat TO <lt_data>.
